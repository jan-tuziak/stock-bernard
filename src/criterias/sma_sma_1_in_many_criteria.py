...
class SmaSma1InManyCriteria(ICriteria):
    ...
    def check_symbol(self, warehouse, symbol):
        ...
        i=0
        for _ in smas_x:
            if smas_x[i] > smas_y[i]: return True
            i+=1
        return False
    ...
    def add_needed_data(self, warehouse, data_source):
        for symbol in warehouse.get_symbols():
            if not warehouse.is_symbol_rejected(symbol):
                smas_x = data_source.get_smas(symbol, self._interval_x, self._time_period_x, self._lookback)
                smas_y = data_source.get_smas(symbol, self._interval_y, self._time_period_y, self._lookback)
